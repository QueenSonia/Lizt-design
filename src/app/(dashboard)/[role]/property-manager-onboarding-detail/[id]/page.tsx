...
function PropertyManagerOnboardingDetailContent() {
  const { id, role } = useParams();
  const router = useRouter();
  const onboardingId = Array.isArray(id) ? id[0] : id;

  const submission = MOCK_PROPERTY_MANAGER_ONBOARDING_SUBMISSIONS.find(
    (s) => s.id === onboardingId,
  );
...
export default function PropertyManagerOnboardingDetailPage() {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <PropertyManagerOnboardingDetailContent />
    </Suspense>
  );
}
