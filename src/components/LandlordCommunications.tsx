...
type ComposeStep = "template" | "message" | "tenants";
...
function buildPreviewBody(
  ...
  return templateBody.replace(/\{\{(\d+)\}\}/g, (_, pos) => {
    const p = parseInt(pos, 10);
    const val = (values[p] ?? "").trim();
    if (val) return val;
    const varDef = variables.find(v => v.position === p);
    return `[${varDef?.name || `Variable ${p}`}]`;
  });
}

// ── Compose Modal (3-step: Template → Message → Tenants) ─────────────────────
...
function ComposeModal({
  ...
  const [step, setStep] = useState<ComposeStep>("template");
  ...
  const previewBody = selectedTemplate
    ? buildPreviewBody(selectedTemplate.body, selectedTemplate.variables, varValues)
    : "";

  const allVarsFilled = selectedTemplate
    ? selectedTemplate.variables.every(v => (varValues[v.position] ?? "").trim().length > 0)
    : true;

  const canProceedToMessage = selectedTemplate !== null;
  const canProceedToTenants = broadcastTitle.trim().length > 0 && allVarsFilled;
  ...
  function goBack() {
    if (step === "message") setStep("template");
    else if (step === "tenants") setStep("message");
  }
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
                ...
                {approvedTemplates.length === 0 ? (
                  <div className="py-10 text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-1 leading-relaxed">
                      Go to Template Library, create a template, and submit it for Meta approval before sending a broadcast.
                    </p>
                  </div>
                ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2 mb-1.5">
                          <p className={`text-sm font-semibold ${isSelected ? "text-[#FF5000]" : "text-gray-900"}`}>
                            {t.name}
                          </p>
                          {isSelected && (
                            <div className="w-5 h-5 rounded-full bg-[#FF5000] flex items-center justify-center shrink-0 mt-0.5">
                              <Check className="w-3 h-3 text-white" />
                            </div>
                          )}
                        </div>
                        <p className="text-xs text-gray-500 leading-relaxed line-clamp-2">
                          {t.body.replace(/\{\{\d+\}\}/g, "…").replace(/\n+/g, " ")}
                        </p>
                        {t.variables.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {t.variables.map(v => (
                              <span key={v.position} className="inline-flex items-center px-1.5 py-0.5 bg-amber-100 text-amber-700 text-[11px] font-medium rounded">
                                {v.name || `Var ${v.position}`}
                              </span>
                            ))}
                          </div>
                        )}
              ...
              <div className="space-y-5">
                ...
                {selectedTemplate.variables.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide shrink-0">Fill in variables</p>
                      <div className="h-px flex-1 bg-gray-100" />
                    </div>
                      ...
                      {selectedTemplate.variables.map(v => (
                        <div key={v.position} className="flex items-center gap-3">
                          ...
                          <Input
                            ...
                            onChange={e => setVarValues(prev => ({ ...prev, [v.position]: e.target.value }))}
                            placeholder={v.name || `Variable ${v.position}`}
                ...
                <div>
                  ...
                  <div className="bg-[#DCF8C6] rounded-2xl rounded-tl-sm px-4 py-4 space-y-2">
                    ...
                    <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-line">{previewBody}</p>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex gap-3 shrink-0">
            {step === "template" ? (
              ...
              <Button variant="outline" className="border-gray-200 text-gray-700" onClick={goBack}>
                Back
              </Button>
            )}
            <Button
              ...
              disabled={
                step === "template" ? !canProceedToMessage :
                step === "message"  ? !canProceedToTenants :
                !canSend
              }
              onClick={() => {
                if (step === "template") setStep("message");
                else if (step === "message") setStep("tenants");
                else handleSend();
              }}
            >
...
function MessagesTab({
  ...
  return (
    <>
      ...
      {showCompose && (
        <ComposeModal onClose={() => setShowCompose(false)} onSent={onSent} templates={templates} />
      )}
